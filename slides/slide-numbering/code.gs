  ...
  try {
    ...
    diapositivas.forEach((diapositiva) => {
      if (!diapositiva.isSkipped()) {
        const forma = diapositiva.insertShape(
        ...
        texto.getTextStyle().setFontSize(14).setForegroundColor("#FFFFFF");
    ...
    SlidesApp.getUi().alert(
      "Completado",
      `Se numeraron ${numero - 1} diapositivas.\nDimensiones: ${ancho}x${alto}`,
      SlidesApp.getUi().ButtonSet.OK
    );
  } catch (e) {
    SlidesApp.getUi().alert(
      "Error",
      "No se pudo completar la numeración: " + e.toString(),
      SlidesApp.getUi().ButtonSet.OK
    );
  }
}
